...
static jlong nativeOpen(JNIEnv* env, jclass clazz, jstring pathStr, jint openFlags,
    ...
    if (err != SQLITE_OK) {
        __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "Could not open database");
    ...
    // enable extensions to be loaded
    __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "Opening connection %p with label '%s'", db, label.c_str());
    int ret;
    char *err_msg = NULL;
    char sql[2048];
    sqlite3_enable_load_extension(db, 1);
    __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "extension loading enabled");
    // load libgpkg here
    strcpy(sql, "SELECT load_extension('libgpkg.so')");
    ret = sqlite3_exec(db, sql, NULL, NULL, &err_msg);
    if (ret != SQLITE_OK) {
      __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "load_extension() error: %s\n", err_msg);
      sqlite3_free (err_msg);
      return 0;
    }
    __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "libgpkg loaded as an extension");
