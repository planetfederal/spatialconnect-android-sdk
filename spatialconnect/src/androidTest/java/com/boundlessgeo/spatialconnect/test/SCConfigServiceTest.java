...
public class SCConfigServiceTest extends BaseTestCase {
  ...
  @Test public void testSpatialConnectCanLoadLocalConfigs() {
    ...
    assertEquals(
        "The test config file has 4 stores (1 from the local config plus form, default, and location "
            + "stores.", 4, sc.getDataService().getAllStores().size());
  }

  @Test public void testConfigServiceCanLoadConfigsFromBackendService() {
    SpatialConnect sc = SpatialConnect.getInstance();
    sc.initialize(activity);
    ...
    SCBackendService.configReceived.filter(new Func1<Boolean, Boolean>() {
      @Override public Boolean call(Boolean b) {
        return b;
      }
    }).take(1).timeout(15, TimeUnit.SECONDS).subscribe(testSubscriber);
    testSubscriber.awaitTerminalEvent();
    testSubscriber.assertCompleted();
    testSubscriber.assertValues(true);
    waitForStoreToStart(RIO_GPKG_ID, sc);
    assertEquals(
        "The remote config file defines 3 stores; plus 7 from the remote config; plus the form, default,"
            + " and location stores", 13, sc.getDataService().getAllStores().size());
    assertEquals("The stores from the remote config are running.",
        SCDataStoreStatus.SC_DATA_STORE_RUNNING,
        sc.getDataService().getStoreById(RIO_GPKG_ID).getStatus());
  }
