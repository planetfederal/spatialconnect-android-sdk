...
public class GeoPackageTest extends BaseTestCase {
  ...
  private static void waitForStoreToStart(final String storeId) {
    ...
    Observable.create(new Observable.OnSubscribe<Void>() {
      @Override public void call(final Subscriber<? super Void> subscriber) {
        ...
        if (dataService.getStoreById(storeId)
            .getStatus()
          ...
          dataService.storeEvents.autoConnect()
              ...
              .subscribe(new Action1<SCStoreStatusEvent>() {
                @Override public void call(SCStoreStatusEvent event) {
                  if (event.getStoreId().equals(storeId) && event.getStatus()
                      .equals(SCDataStoreStatus.SC_DATA_STORE_RUNNING)) {
                    subscriber.onCompleted();
                  }
                }
  ...
  @Test public void testThatDataServiceStartedGeoPackageStore() {
    ...
    assertEquals("The store should be running", SCDataStoreStatus.SC_DATA_STORE_RUNNING,
        gpkgStore.getStatus());
  }

  @Test public void testGeoPackageQueryStore() {
    // bbox around part of rio
    SCBoundingBox bbox =
        new SCBoundingBox(-43.413848877, -23.0077002267, -43.150177002, -22.7951732956);
    SCQueryFilter filter = new SCQueryFilter(
        new SCPredicate(bbox, SCGeometryPredicateComparison.SCPREDICATE_OPERATOR_WITHIN));
    TestSubscriber testSubscriber = new TestSubscriber();
    sc.getDataService()
        .queryStores(Arrays.asList(RIO_GPKG_ID), filter)
        .timeout(5, TimeUnit.SECONDS)
        .subscribe(testSubscriber);
    testSubscriber.awaitTerminalEvent();
    assertTrue("The query should have returned some features.",
        testSubscriber.getOnNextEvents().size() > 0);
  }

  @Test public void testGeoPackageQueryWithin() {
    ...
    //testSubscriber.assertNoErrors();
    assertEquals(
        "The query should have the first 100 features, b/c we specify a layer without a limit.",
        (Integer) 100, (Integer) testSubscriber.getOnNextEvents().size());
    SCSpatialFeature feature = (SCSpatialFeature) testSubscriber.getOnNextEvents().get(0);
    assertEquals("The store id should be for Rio.", RIO_GPKG_ID, feature.getKey().getStoreId());
    assertEquals("The layer should be police_stations.", "police_stations",
        feature.getKey().getLayerId());
    assertEquals("The id should match the featureId from the key tuple.", feature.getId(),
        feature.getKey().getFeatureId());
    assertTrue("The feature should have properties.", feature.getProperties().size() > 0);
  }

  @Test public void testGeoPackageQueryContains() {
    // this is the geopackage located at https://s3.amazonaws.com/test.spacon/rio.gpkg
    SCDataStore gpkgStore = sc.getDataService().getStoreById(RIO_GPKG_ID);
  ...
  @Test public void testGeoPackageQueryById() {
    SCDataStore gpkgStore = sc.getDataService().getStoreById(RIO_GPKG_ID);
    ...
    testSubscriber.awaitTerminalEvent();
    assertEquals("The query should have returned 1 feature", (Integer) 1,
        (Integer) testSubscriber.getOnNextEvents().size());
    SCSpatialFeature feature = (SCSpatialFeature) testSubscriber.getOnNextEvents().get(0);
    assertEquals("The id of the returned feature should be 1.", "1",
        feature.getKey().getFeatureId());
  }

  @Test public void testGeoPackageCreateFeature() {
    SCDataStore gpkgStore = sc.getDataService().getStoreById(RIO_GPKG_ID);
  ...
  @Test public void testGeoPackageUpdateFeature() {
  ...
  @Test public void testGeoPackageDeleteFeature() {
    final SCDataStore gpkgStore = sc.getDataService().getStoreById(RIO_GPKG_ID);
    ...
    gpkgStore.queryById(pointToDelete.getKey())
        .timeout(5, TimeUnit.SECONDS)
        .subscribe(testSubscriber);
    testSubscriber.awaitTerminalEvent();
    assertEquals("No results should be returned b/c the feature should be deleted", 0,
        testSubscriber.getOnNextEvents().size());
  }

  @Test public void testGeoPackageCreateFeatureThrowsDataStoreExceptionWhenNoFeatureTablesExist() {
    SCDataStore gpkgStore = sc.getDataService().getStoreById(RIO_GPKG_ID);
    TestSubscriber testSubscriber = new TestSubscriber();
  ...
  @Test public void testGeoPackageUpdateFeatureThrowsDataStoreExceptionWhenNoFeatureTablesExist() {
    SCDataStore gpkgStore = sc.getDataService().getStoreById(RIO_GPKG_ID);
    TestSubscriber testSubscriber = new TestSubscriber();
    SCSpatialFeature feature = getTestRioPoint();
    feature.setLayerId("invalid_table_name");
  ...
  @Test public void testGeoPackageDeleteFeatureThrowsDataStoreExceptionWhenNoFeatureTablesExist() {
    SCDataStore gpkgStore = sc.getDataService().getStoreById(RIO_GPKG_ID);
    TestSubscriber testSubscriber = new TestSubscriber();
    SCSpatialFeature feature = getTestRioPoint();
    feature.setLayerId("invalid_table_name");
  ...
  @Test public void testGeoPackageQueryByIdThrowsDataStoreExceptionWhenNoFeatureTablesExist() {
    SCDataStore gpkgStore = sc.getDataService().getStoreById(RIO_GPKG_ID);
    TestSubscriber testSubscriber = new TestSubscriber();
    SCSpatialFeature feature = getTestRioPoint();
    feature.setLayerId("invalid_table_name");
  ...
  @Ignore public void testInvalidGeoPackageIsNotStarted() {
    ...
    Observable.create(new Observable.OnSubscribe<Void>() {
      @Override public void call(final Subscriber<? super Void> subscriber) {
        sc.getDataService().storeEvents.autoConnect()
            ...
            .subscribe(new Action1<SCStoreStatusEvent>() {
              @Override public void call(SCStoreStatusEvent event) {
                if (event.getStoreId().equals(HAITI_GPKG_ID) && event.getStatus()
                    .equals(SCDataStoreStatus.SC_DATA_STORE_STOPPED)) {
                  subscriber.onCompleted();
                }
              }
            });
