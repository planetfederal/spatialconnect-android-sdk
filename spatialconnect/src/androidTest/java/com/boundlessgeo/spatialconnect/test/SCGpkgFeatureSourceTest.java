...
@Ignore public class SCGpkgFeatureSourceTest extends BaseTestCase {
  ...
  @Test public void testGetGeoPackageContents() {
    ...
    assertEquals("The Whitehorse gpkg should only have 1 row in the gpkg_contents table.", 1,
        contentsSize);
  }

  @Test public void testGetFeatureSources() {
  ...
  @Test public void testRtreeIndexIsCreated() {
    ...
    for (SCGpkgFeatureSource source : ((GeoPackageAdapter) store.getAdapter()).getFeatureSources()
        .values()) {
      try {
        cursor = db.query("SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name=?",
