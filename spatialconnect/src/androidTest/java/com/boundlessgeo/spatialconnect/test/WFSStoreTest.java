...
public class WFSStoreTest extends BaseTestCase {
  ...
  @Test public void testDataServiceStartsWFSStoreCorrectly() {
    ...
    assertEquals("The wfs store should be running", SCDataStoreStatus.SC_DATA_STORE_RUNNING,
        wfsStore.getStatus());
  }

  @Test public void testGetCapabilitesUrlIsBuiltCorrectly() {
  ...
  @Test public void testLayerNamesAreParsedFromCapabilities() {
    WFSStore store = (WFSStore) sc.getDataService().getStoreById(WFS_STORE_ID);
    assertTrue("There should be multiple layers.", store.getLayerNames().size() > 0);
  }

  @Test public void testQueryWithBbox() {
    ...
    assertTrue("The query observable should have emitted features",
