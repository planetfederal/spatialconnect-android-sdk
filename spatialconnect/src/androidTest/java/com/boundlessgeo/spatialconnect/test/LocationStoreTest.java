...
public class LocationStoreTest extends BaseTestCase {
  ...
  @Test public void testLocationStoreIsInitialized() {
    ...
    assertEquals("The location store should have the adapter connected if it initialized properly.",
        locationStore.getAdapter().getStatus(), SCDataAdapterStatus.DATA_ADAPTER_CONNECTED);

    assertEquals("The location store should have 1  table.", 1,
        ((GeoPackageAdapter) locationStore.getAdapter()).getGeoPackageContents().size());
  }

  @Test public void testAddingLocation() {
