...
public class SCSqliteHelperTest extends BaseTestCase {
  ...
  @Test public void testGeoPackageStoreCanBeAccessedBySCSqliteHelper() {
  ...
  @Test public void test_CreateSpatialIndex_FunctionLoaded() {
    Cursor cursor = rio.query("SELECT CreateSpatialIndex(?, ?, ?);",
        new String[] { "police_stations", "geom", "fid" });
    cursor.moveToFirst();
  ...
  @Test public void test_ST_AsText_FunctionLoaded() {
    Cursor cursor = rio.query("SELECT ST_AsText(geom) FROM police_stations LIMIT 1;");
    cursor.moveToFirst();
    assertTrue("The geom should start with Point if ST_AsText worked.",
        cursor.getString(0).startsWith("Point"));
  }

  @Test public void test_ST_GeomFromText_FunctionLoaded() {
    Cursor cursor = rio.query("SELECT ST_GeomFromText('Point (-72.981321 18.42740796)');");
    cursor.moveToFirst();
    assertTrue("ST_GeomFromText should return a blob if it loaded correctly.",
        cursor.getColumnCount() == 1);
    assertTrue("ST_GeomFromText should return a blob if it loaded correctly.",
        cursor.getBlob(0).length > 0);
  }

  @Test public void test_ST_WKTToSQL_FunctionLoaded() {
    Cursor cursor = rio.query("SELECT ST_WKTToSQL('Point (0 0)');");
    cursor.moveToFirst();
    assertTrue("ST_WKTToSQL should return a blob if it loaded correctly.",
        cursor.getColumnCount() == 1);
    assertTrue("ST_WKTToSQL should return a blob if it loaded correctly.",
        cursor.getBlob(0).length > 0);
  }
  ...
  @Test public void test_ST_MinX_FunctionLoaded() {
  ...
  @Test public void test_ST_MaxX_FunctionLoaded() {
  ...
  @Test public void test_ST_MinY_FunctionLoaded() {
  ...
  @Test public void test_ST_MaxY_FunctionLoaded() {
  ...
  @Test public void test_ST_SRID_FunctionLoaded() {
    Cursor cursor = rio.query("SELECT ST_SRID(geom) FROM police_stations LIMIT 1;");
    cursor.moveToFirst();
    assertEquals("ST_SRID should return 4326 geom column in police_stations.", 4326,
        cursor.getInt(0));
  }

  @Test public void test_ST_IsMeasured_FunctionLoaded() {
  ...
  @Test public void test_ST_Is3d_FunctionLoaded() {
  ...
  @Test public void test_ST_CoordDim_FunctionLoaded() {
  ...
  @Test public void test_ST_GeometryType_FunctionLoaded() {
