...
public class ServicesTest extends BaseTestCase {
  ...
  @Test public void testDataServiceInitialization() {
    SpatialConnect.getInstance().initialize(testContext);
    SCDataService dataService = SpatialConnect.getInstance().getDataService();
    assertEquals("The data service should have 5 supported stores.", 5,
        dataService.getSupportedStoreKeys().size());
  }

  @Test public void testSpatialConnectSetup() {
    SpatialConnect.getInstance().initialize(testContext);
    SCDataService dataService = SpatialConnect.getInstance().getDataService();
    ...
    assertEquals(
        "5 default services should have been initialized (data, kvp, sensor, config, and auth)", 5,
        sc.getServices().size());
    assertEquals(
        "There should be only 5 supported data stores: geojson.1, gpkg.1 and geojson.1.0, gpkg.1.0, "
            + "wfs.1.1.0", 5, sc.getDataService().getSupportedStoreKeys().size());
