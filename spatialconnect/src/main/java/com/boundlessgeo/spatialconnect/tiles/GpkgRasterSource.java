...
public class GpkgRasterSource implements SCRasterStore {
  ...
  @Override public void overlayFromLayer(String layerName, GoogleMap map) {
    Map<String, SCGpkgTileSource> tileSources =
        ((GeoPackageAdapter) gpkgStore.getAdapter()).getTileSources();
  ...
  @Override public SCPolygon getCoverage() {
    return null;
  }

  @Override public List<SCGpkgTileSource> rasterList() {
    return new ArrayList<SCGpkgTileSource>(
        ((GeoPackageAdapter) gpkgStore.getAdapter()).getTileSources().values());
  }
}
