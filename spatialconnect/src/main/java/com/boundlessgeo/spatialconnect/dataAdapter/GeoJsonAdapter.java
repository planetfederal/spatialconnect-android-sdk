...
public class GeoJsonAdapter extends SCDataAdapter {
        ...
        if (!geoJsonFile.exists()) {
          if (scStoreConfig.getUri().startsWith("http")) {
            ...
            try {
              ...
              HttpHandler.getInstance().get(theUrl.toString()).subscribe(new Action1<Response>() {
            ...
            if (!f.exists()) {
              ...
              try {
                Log.d(LOG_TAG,
                    "Attempting to connect to " + scStoreConfig.getUri().split("\\.")[0]);
                int resourceId = context.getResources()
                    .getIdentifier(scStoreConfig.getUri().split("\\.")[0], "raw",
  ...
  public Observable<SCSpatialFeature> query(final SCQueryFilter filter) {
    ...
    final SCGeometryCollection collection =
        factory.getGeometryCollectionFromFeatureCollectionJson(getResourceAsString());

    return Observable.from(collection.getFeatures()).filter(new Func1<SCSpatialFeature, Boolean>() {
      @Override public Boolean call(SCSpatialFeature feature) {
      ...
      @Override public SCSpatialFeature call(SCSpatialFeature scSpatialFeature) {
  ...
  private InputStream getResourceStream() {
    int resourceId = context.getResources()
        .getIdentifier(scStoreConfig.getUri().split("\\.")[0], "raw", context.getPackageName());
