...
public class GeoPackageAdapter extends SCDataAdapter {
  ...
  @Override public Observable<SCDataAdapterStatus> connect() {
    ...
    return Observable.create(new Observable.OnSubscribe<SCDataAdapterStatus>() {
      ...
      @Override public void call(final Subscriber<? super SCDataAdapterStatus> subscriber) {

        Log.d(LOG_TAG,
            "Connecting to " + context.getDatabasePath(scStoreConfig.getUniqueID()).getPath());
        ...
        if (context.getDatabasePath(scStoreConfig.getUniqueID()).exists()) {
          Log.d(LOG_TAG,
              "GeoPackage " + scStoreConfig.getUniqueID() + " already exists.  Not downloading.");
          ...
          if (scStoreConfig.getUri().startsWith("http")) {
            try {
              ...
              downloadGeoPackage(theUrl).subscribe(new Action1<Response>() {
  ...
  public void addLayer(String layer, Map<String, String> typeDefs) {
    if (!layerExists(layer)) {
      ...
      try {
        ...
        cursor = gpkg.query(
            String.format("SELECT AddGeometryColumn('%s', 'geom', 'Geometry', 4326)", tableName));
        cursor.moveToFirst(); // force query to execute
        tx.markSuccessful();
      } catch (Exception ex) {
        ex.printStackTrace();
  ...
  private boolean layerExists(String layer) {
    ...
    String table = layer;
    Cursor cursor = gpkg.query("SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name=?",
        new String[] { table });
    try {
  ...
  public Observable<SCSpatialFeature> query(final SCQueryFilter queryFilter) {
    ...
    if (layers.size() > 0) {  // ensure only layers with feature sources are queried
      ...
      final List<String> featureTableNames =
          queryFilter.getLayerIds().size() > 0 ? queryFilter.getLayerIds()
      ...
      return Observable.from(featureTableNames).filter(new Func1<String, Boolean>() {
        @Override public Boolean call(String featureTableName) {
          return layers.containsKey(featureTableName);
        }
      }).flatMap(new Func1<String, Observable<SCSpatialFeature>>() {
        @Override public Observable<SCSpatialFeature> call(final String layerName) {
          final SCGpkgFeatureSource featureSource = gpkg.getFeatureSourceByName(layerName);
          return gpkg.createQuery(layerName,
              String.format("SELECT %s FROM %s WHERE %s IN (%s) LIMIT %d",
                  getSelectColumnsString(featureSource), layerName,
  ...
  public Observable<SCSpatialFeature> queryById(final SCKeyTuple keyTuple) {
    ...
    if (featureSource == null) {
      ...
      return gpkg.createQuery(tableName, String.format("SELECT %s FROM %s WHERE %s = %s LIMIT 1",
          getSelectColumnsString(featureSource), tableName, featureSource.getPrimaryKeyName(),
          keyTuple.getFeatureId())).flatMap(getFeatureMapper(featureSource));
    }
  }
  ...
  public Observable<SCSpatialFeature> create(final SCSpatialFeature scSpatialFeature) {
    ...
    if (featureSource == null) {
      ...
      return Observable.create(new Observable.OnSubscribe<SCSpatialFeature>() {
        @Override public void call(Subscriber<? super SCSpatialFeature> subscriber) {
          try {
            gpkg.executeAndTrigger(tableName,
                String.format("INSERT OR REPLACE INTO %s (%s) VALUES (%s)", tableName,
    ...
    if (featureSource == null) {
      ...
      return Observable.create(new Observable.OnSubscribe<SCSpatialFeature>() {
        @Override public void call(Subscriber<? super SCSpatialFeature> subscriber) {
          try {
            gpkg.executeAndTrigger(tableName,
                String.format("UPDATE %s SET %s WHERE %s = %s", tableName,
                    featureSource.getUpdateSetClause(scSpatialFeature),
                    featureSource.getPrimaryKeyName(), scSpatialFeature.getId()));
            subscriber.onNext(scSpatialFeature);
            subscriber.onCompleted();
          } catch (SQLException ex) {
    ...
    if (featureSource == null) {
      ...
      return Observable.create(new Observable.OnSubscribe<Void>() {
        @Override public void call(Subscriber<? super Void> subscriber) {
          try {
            gpkg.executeAndTrigger(tableName,
                String.format("DELETE FROM %s WHERE %s = %s", tableName,
                    featureSource.getPrimaryKeyName(), keyTuple.getFeatureId()));
            subscriber.onCompleted();
          } catch (SQLException ex) {
   ...
   * @return
   */
  public Func1<SqlBrite.Query, Observable<SCSpatialFeature>> getFeatureMapper(
      final SCGpkgFeatureSource source) {
    return new Func1<SqlBrite.Query, Observable<SCSpatialFeature>>() {

      @Override public Observable<SCSpatialFeature> call(SqlBrite.Query query) {
        return query.asRows(new Func1<Cursor, SCSpatialFeature>() {
          @Override public SCSpatialFeature call(final Cursor cursor) {
            ...
            try {
              if (wkb != null || wkb.length > 0) {
                feature = new SCGeometry(new WKBReader(GEOMETRY_FACTORY).read(wkb));
              }
            ...
            for (Map.Entry<String, String> column : source.getColumns().entrySet()) {
              if (column.getValue().equalsIgnoreCase("BLOB")
                ...
                feature.getProperties()
                    .put(column.getKey(), SCSqliteHelper.getBlob(cursor, column.getKey()));
              } else if (column.getValue().startsWith("INTEGER")) {
                feature.getProperties()
                    .put(column.getKey(), SCSqliteHelper.getInt(cursor, column.getKey()));
              } else if (column.getValue().startsWith("REAL")) {
                feature.getProperties()
                    .put(column.getKey(), SCSqliteHelper.getLong(cursor, column.getKey()));
              } else if (column.getValue().startsWith("TEXT")) {
                feature.getProperties()
                    .put(column.getKey(), SCSqliteHelper.getString(cursor, column.getKey()));
              } else {
                Log.w(LOG_TAG,
                    "The column type " + column.getValue() + " did not match any supported" +
  ...
  private ContentValues getContentValues(SCSpatialFeature scSpatialFeature) {
    ...
    for (Map.Entry<String, Object> prop : scSpatialFeature.getProperties().entrySet()) {
      ...
      if (columnType.startsWith("TEXT")) {
        ...
        Log.w(LOG_TAG,
            "The column type " + columnType + " did not match any supported column type.");
      }
    }
    return values;
  }
  ...
  private String createRtreeSubQuery(SCGpkgFeatureSource source, SCBoundingBox bbox) {
    return String.format(
        "SELECT id FROM rtree_%s_%s WHERE minx > %f AND maxx < %f AND miny > %f AND maxy < %f",
        source.getTableName(), source.getGeomColumnName(), bbox.getMinX(), bbox.getMaxX(),
  ...
  private String addToGpkgContentsSQL(String tableName) {
    ...
    sb.append("INSERT OR REPLACE INTO gpkg_contents ")
        ...
        .append(
            String.format("VALUES ('%s','features','%s','%s',0,0,0,0,4326)", tableName, tableName,
                tableName));
    return sb.toString();
  }
}
