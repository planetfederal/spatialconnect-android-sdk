...
/**
 * This class is responsible for reading and writing {@link SCSpatialFeature}s to a feature table in
 * a GeoPackage.
 */
public class SCGpkgFeatureSource {
  ...
  /**
   * Creates and instance of the {@link SCGpkgFeatureSource} using the {@link GeoPackage} containing
   * the db
  ...
  /**
   * Builds the column names part of the INSERT statement.  Only adds the columns from the feature,
   * which may not be
   * all columns in the schema.  Note that the INSERT statement doesn't use the primary key column
   * b/c it should be
  ...
  /**
   * Builds the column values part of the INSERT statement.  In order to ensure that the columns
   * match the values, we
  ...
  public String getColumnValuesForInsert(final SCSpatialFeature feature) {
    ...
    for (String columnName : columnNames) {
      if (feature.getProperties().get(columnName) != null) {
        ...
        sb.append(
            DatabaseUtils.sqlEscapeString(String.valueOf(feature.getProperties().get(columnName))));
    ...
    for (String columnName : columns.keySet()) {
      if (feature.getProperties().get(columnName) != null) {
        ...
        sb.append((DatabaseUtils.sqlEscapeString(
            String.valueOf(feature.getProperties().get(columnName)))));
        sb.append(", ");
      }
    }
    sb.append(geomColumnName);
