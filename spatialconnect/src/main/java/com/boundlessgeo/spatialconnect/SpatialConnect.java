...
import com.boundlessgeo.spatialconnect.services.backendService.ISCBackend;
...
public class SpatialConnect {
    ...
    /**
     * Connects to the backend using an implementation of ISCBackend defined by the config.
     */
    public void connectBackend(ISCBackend backend) {
        Log.v(LOG_TAG, String.format("connecting to backend %s", backend.getBackendUri()));
        backendService = new SCBackendService(context, backend);
