...
/**
 * The SCConfigService is responsible for managing the configuration for SpatialConnect.  This
 * includes downloading
 * remote configuration and sweeping the external storage for config files, if required. The config
 * service is
 * also responsible for parsing the relevant parts of the config and invoking functions on other
 * services using parts
...
public class SCConfigService extends SCService {
   ...
   * <p/>
   * <p>First, we try to load the default config from internal storage, then we try to load other
   * configs from the
   * external storage folder.  Note that both internal and external storage supports the "no
   * network" use case
   * where the SDcard has data on it and the config file points to that data.  Next, we try to
   * download any
   * configuration files from the SpatialConnect backend service defined in the {@code remote}
   * attribute of a
   ...
   * <a href="http://developer.android.com/guide/topics/data/data-storage.html#filesExternal">external
   * storage</a>
  ...
  /**
   * Loads the default configuration file from the app's internal files directory.  Note that the
   * configs.scfg file
  ...
  /**
   * If application developers want to store their configs within the APK itself, they can add the
   * config using
