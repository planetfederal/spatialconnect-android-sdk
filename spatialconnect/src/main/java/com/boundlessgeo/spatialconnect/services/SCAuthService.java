...
public class SCAuthService extends SCService {
            ...
            HttpHandler.getInstance()
                ...
                .subscribe(new Action1<Response>() {
                  @Override public void call(Response response) {
                    try {
                      accessToken = new JSONObject(response.body().string()).getJSONObject("result")
                          .getString("token");
                      if (accessToken != null) {
                  ...
                  @Override public void call(Throwable throwable) {
                    Log.e(LOG_TAG,
                        "something went wrong refreshing token: " + throwable.getMessage());
                  }
                });
          }
        }
  ...
  public static class AuthHeaderInterceptor implements Interceptor {
    @Override public Response intercept(Interceptor.Chain chain) throws IOException {
      ...
      if (originalRequest.header(AUTH_HEADER_NAME) != null || originalRequest.toString()
      ...
      if (accessToken == null) {
        ...
        Request updatedRequest =
            originalRequest.newBuilder().header(AUTH_HEADER_NAME, accessToken).build();
  ...
  public static class SCAuthenticator implements Authenticator {
    @Override public Request authenticate(Route route, Response response) throws IOException {
      return response.request().newBuilder().addHeader(AUTH_HEADER_NAME, getAccessToken()).build();
    }
  }
}
