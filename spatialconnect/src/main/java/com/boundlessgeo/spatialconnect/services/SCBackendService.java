...
public class SCBackendService extends SCService {
  ...
  public void initialize(SCRemoteConfig config) {
    if (API_URL == null) {
      API_URL = String.format("%s://%s:%s/api/", config.getHttpProtocol(), config.getHttpHost(),
  ...
  @Override public void start() {
    ...
    SCAuthService.loginStatus.subscribe(new Action1<Boolean>() {
      @Override public void call(Boolean authenticated) {
        if (authenticated) {
  ...
  private void setupSubscriptions() {
    notifications = listenOnTopic("/notify").flatMap(
        new Func1<SCMessageOuterClass.SCMessage, Observable<SCNotification>>() {
          @Override public Observable<SCNotification> call(SCMessageOuterClass.SCMessage message) {
  ...
  private void fetchConfig() {
    ...
    SCMessageOuterClass.SCMessage getConfigMsg =
        SCMessageOuterClass.SCMessage.newBuilder().setAction(SCCommand.CONFIG_FULL.value()).build();
    publishReplyTo("/config", getConfigMsg).subscribe(new Action1<SCMessageOuterClass.SCMessage>() {
      @Override public void call(SCMessageOuterClass.SCMessage scMessage) {
  ...
  private void registerDevice() {
    SCMessageOuterClass.SCMessage registerConfigMsg = SCMessageOuterClass.SCMessage.newBuilder()
        ...
        .setPayload(String.format("{\"identifier\": \"%s\", \"device_info\": \"%s\"}",
            SCConfigService.getClientId(), SCConfigService.getAndroidVersion()))
        .build();
        ...
        switch (SCCommand.fromActionNumber(scMessage.getAction())) {
          case CONFIG_ADD_STORE:
            try {
              SCStoreConfig config =
                  ObjectMappers.getMapper().readValue(scMessage.getPayload(), SCStoreConfig.class);
              SpatialConnect.getInstance().getDataService().registerAndStartStoreByConfig(config);
            } catch (IOException e) {
              e.printStackTrace();
            }
            break;
          case CONFIG_UPDATE_STORE:
            try {
              SCStoreConfig config =
                  ObjectMappers.getMapper().readValue(scMessage.getPayload(), SCStoreConfig.class);
              SpatialConnect.getInstance().getDataService().updateStoresByConfig(config);
            } catch (IOException e) {
              e.printStackTrace();
            }
            break;
          case CONFIG_REMOVE_STORE:
            SCDataStore store =
                SpatialConnect.getInstance().getDataService().getStoreById(scMessage.getPayload());
            SpatialConnect.getInstance().getDataService().unregisterStore(store);
            break;
          case CONFIG_ADD_FORM:
            try {
              SCFormConfig config =
                  ObjectMappers.getMapper().readValue(scMessage.getPayload(), SCFormConfig.class);
              SpatialConnect.getInstance()
                  .getDataService()
                  .getFormStore()
                  .registerFormByConfig(config);
            } catch (IOException e) {
              e.printStackTrace();
            }
            break;
  ...
  public Observable<SCMessageOuterClass.SCMessage> listenOnTopic(final String topic) {
    ...
    return mqttHandler.scMessageSubject.filter(
        new Func1<Map<String, SCMessageOuterClass.SCMessage>, Boolean>() {
          @Override
        ...
        .flatMap(
            new Func1<Map<String, SCMessageOuterClass.SCMessage>, Observable<SCMessageOuterClass.SCMessage>>() {
              @Override public Observable<SCMessageOuterClass.SCMessage> call(
                  Map<String, SCMessageOuterClass.SCMessage> stringSCMessageMap) {
   ...
   */
  public Observable<SCMessageOuterClass.SCMessage> publishReplyTo(String topic,
    ...
    return listenOnTopic(MqttHandler.REPLY_TO_TOPIC).filter(
        new Func1<SCMessageOuterClass.SCMessage, Boolean>() {
          @Override public Boolean call(SCMessageOuterClass.SCMessage incomingMessage) {
            return incomingMessage.getCorrelationId() == newMessage.getCorrelationId();
          }
        })
        .flatMap(
            new Func1<SCMessageOuterClass.SCMessage, Observable<SCMessageOuterClass.SCMessage>>() {
              @Override public Observable<SCMessageOuterClass.SCMessage> call(
                  SCMessageOuterClass.SCMessage message) {
  ...
  private void loadConfig(SCMessageOuterClass.SCMessage message) {
    ...
    try {
      SCConfig config = ObjectMappers.getMapper().readValue(message.getPayload(), SCConfig.class);
