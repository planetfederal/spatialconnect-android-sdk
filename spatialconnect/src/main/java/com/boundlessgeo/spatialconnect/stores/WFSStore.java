...
public class WFSStore extends SCDataStore {
  ...
  @Override public Observable<SCSpatialFeature> query(SCQueryFilter scFilter) {
    // if there are no layer names supplied in the query filter, then search only on the default layers
    final List<String> layerNames =
        scFilter.getLayerIds().size() > 0 ? scFilter.getLayerIds() : getDefaultLayers();
    ...
    String getFeatureUrl = String.format("%s?service=WFS&version=%s&request=GetFeature&typeName=%s"
            + "&outputFormat=application/json&srsname=EPSG:4326&maxFeatures=%d", baseUrl, getVersion(),
        TextUtils.join(",", layerNames), scFilter.getLimit());
    if (scFilter.getPredicate() != null) {
      getFeatureUrl = String.format("%s&bbox=%f,%f,%f,%f,EPSG:4326", getFeatureUrl,
              ...
              try {
                ...
                for (SCSpatialFeature feature : collection.getFeatures()) {
                  feature.setLayerId(feature.getId()
                      .split("\\.")[0]);  // the first part of the id is the layer name
  ...
  @Override public Observable queryById(SCKeyTuple keyTuple) {
    return null;
  }

  @Override public Observable create(SCSpatialFeature scSpatialFeature) {
    return null;
  }

  @Override public Observable update(SCSpatialFeature scSpatialFeature) {
    return null;
  }

  @Override public Observable delete(SCKeyTuple keyTuple) {
    return null;
  }

  @Override public Observable<SCStoreStatusEvent> start() {
    ...
    return Observable.create(new Observable.OnSubscribe<SCStoreStatusEvent>() {
      @Override public void call(final Subscriber<? super SCStoreStatusEvent> subscriber) {
  ...
  private List<String> parseLayerNames(XmlPullParser parser)
  ...
  public String getGetCapabilitiesUrl() {
    return String.format("%s?service=WFS&version=%s&request=GetCapabilities", baseUrl,
        this.getVersion());
  }
