...
public class GeoPackageStore extends SCDataStore {
  ...
  @Override public DataStorePermissionEnum getAuthorization() {
    return DataStorePermissionEnum.READ_WRITE;
  }

  @Override public Observable<SCSpatialFeature> query(final SCQueryFilter scFilter) {
    return ((GeoPackageAdapter) getAdapter()).query(scFilter);
  }

  @Override public Observable<SCSpatialFeature> queryById(final SCKeyTuple keyTuple) {
    return ((GeoPackageAdapter) getAdapter()).queryById(keyTuple);
  }

  @Override public Observable<SCSpatialFeature> create(final SCSpatialFeature scSpatialFeature) {
    return ((GeoPackageAdapter) getAdapter()).create(scSpatialFeature);
  }

  @Override public Observable<SCSpatialFeature> update(final SCSpatialFeature scSpatialFeature) {
    return ((GeoPackageAdapter) getAdapter()).update(scSpatialFeature);
  }

  @Override public Observable<Void> delete(final SCKeyTuple keyTuple) {
    ...
    return Observable.create(new Observable.OnSubscribe<SCStoreStatusEvent>() {
      @Override public void call(final Subscriber<? super SCStoreStatusEvent> subscriber) {
        ...
        storeInstance.getAdapter().connect().subscribe(new Subscriber<SCDataAdapterStatus>() {
          ...
          @Override public void onNext(SCDataAdapterStatus status) {
          }
        });
      }
    });
