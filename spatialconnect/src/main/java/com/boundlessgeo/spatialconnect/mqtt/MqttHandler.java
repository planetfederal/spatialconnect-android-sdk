...
public class MqttHandler implements MqttCallback {
  ...
  public final static String REPLY_TO_TOPIC =
      String.format("/device/%s-replyTo", SCConfigService.getClientId());
  ...
  public PublishSubject<Map<String, SCMessageOuterClass.SCMessage>> scMessageSubject =
  ...
  private String getMqttBrokerUri(SCRemoteConfig config) {
    return String.format("%s://%s:%s", config.getMqttProtocol(), config.getMqttHost(),
   ...
   * @param qos     quality of service (0, 1, 2)
   */
  public void publish(final String topic, final SCMessageOuterClass.SCMessage message,
      final int qos) {
    clientConnected.subscribe(new Action1<Integer>() {
      @Override public void call(Integer integer) {
        if (integer == 1) {
  ...
  @Override public void messageArrived(String topic, MqttMessage message) throws Exception {
    ...
    SCMessageOuterClass.SCMessage scMessage =
        SCMessageOuterClass.SCMessage.parseFrom(message.getPayload());
  ...
  class ConnectActionListener implements IMqttActionListener {
    ...
    @Override public void onFailure(IMqttToken asyncActionToken, Throwable ex) {
