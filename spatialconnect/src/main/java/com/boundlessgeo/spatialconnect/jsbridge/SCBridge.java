...
public class SCBridge extends ReactContextBaseJavaModule {
  ...
  @ReactMethod public void bindMapView(final int tag) {
          ...
          @Override public void onMapReady(GoogleMap googleMap) {
            ...
            for (SCDataStore store : stores) {
              if (store instanceof GeoPackageStore
                  && ((GeoPackageAdapter) store.getAdapter()).getTileSources().size() > 0) {
  ...
  public void sendEvent(Integer eventType, @Nullable WritableMap params) {
    Log.v(LOG_TAG,
        String.format("Sending {\"type\": %s, \"payload\": %s} to Javascript", eventType.toString(),
            params.toString()));
  ...
  public void sendEvent(Integer eventType, String responseId, @Nullable WritableMap params) {
    if (eventType != 113) {
      Log.v(LOG_TAG, String.format(
          "Sending {\"responseID\": \"%s\", \"type\": %s, \"payload\": %s} to Javascript",
          responseId, eventType.toString(), params.toString()));
    }
    WritableMap newAction = Arguments.createMap();
    newAction.putInt("type", eventType);
    newAction.putMap("payload", params);
  ...
  public void sendEvent(Integer eventType, String responseId, @Nullable String payloadString) {
    if (eventType != 113) {
      Log.v(LOG_TAG, String.format(
          "Sending {\"responseID\": \"%s\", \"type\": %s, \"payload\": %s} to Javascript",
          responseId, eventType.toString(), payloadString));
    }
    WritableMap newAction = Arguments.createMap();
    newAction.putInt("type", eventType);
  ...
  public void sendEvent(Integer eventType, @Nullable String payloadString) {
    Log.v(LOG_TAG, String.format("Sending {\"type\": %s, \"payload\": \"%s\"} to Javascript",
        eventType.toString(), payloadString));
    WritableMap newAction = Arguments.createMap();
    newAction.putInt("type", eventType);
    newAction.putString("payload", payloadString);
    this.reactContext.getJSModule(DeviceEventManagerModule.RCTDeviceEventEmitter.class)
  ...
  public void sendEvent(Integer eventType, @Nullable Integer payloadInteger) {
    Log.v(LOG_TAG,
        String.format("Sending {\"type\": %s, \"payload\": %d} to Javascript", eventType.toString(),
            payloadInteger));
    WritableMap newAction = Arguments.createMap();
    newAction.putInt("type", eventType);
   ...
   * @param message
   */
  @ReactMethod public void handler(ReadableMap message) {
    ...
    if (message == null && message.equals("undefined")) {
      ...
      if (command.equals(SCCommand.DATASERVICE_GEOSPATIALQUERY) || command.equals(
          SCCommand.DATASERVICE_SPATIALQUERY)) {
        handleQuery(message);
      }
        ...
        if (connected) {
          SpatialConnect.getInstance()
              ...
              .subscribe(new Action1<SCNotification>() {
                @Override public void call(SCNotification scNotification) {
                  try {
  ...
  private void handleActiveStoresList(final ReadableMap message) {
    ...
    sc.getDataService().hasStores.subscribe(new Action1<Boolean>() {
      @Override public void call(Boolean hasStores) {
        if (hasStores) {
          sendEvent(message.getInt("type"), message.getString("responseId"),
              getActiveStoresPayload());
        }
      }
    });
  }
  ...
  private void handleFormsList(final ReadableMap message) {
    ...
    sc.getDataService().getFormStore().hasForms.subscribe(new Action1<Boolean>() {
      @Override public void call(Boolean hasForms) {
        if (hasForms) {
  ...
  private void handleQuery(final ReadableMap message) {
    ...
    if (filter != null) {
      sc.getDataService().queryStores(storeIds, filter).subscribeOn(Schedulers.io())
          //                    .onBackpressureBuffer(filter.getLimit())
          .subscribe(new Subscriber<SCSpatialFeature>() {
            ...
            @Override public void onNext(SCSpatialFeature feature) {
  ...
  private void handleUpdateFeature(ReadableMap message) {
    ...
    try {
      ...
      sc.getDataService()
          .getStoreById(featureToUpdate.getKey().getStoreId())
          ...
          .subscribe(new Subscriber<SCSpatialFeature>() {
            ...
            @Override public void onNext(SCSpatialFeature updated) {
  ...
  private void handleDeleteFeature(ReadableMap message) {
    ...
    try {
      ...
      sc.getDataService()
          .getStoreById(featureKey.getStoreId())
          .delete(featureKey)
          .subscribeOn(Schedulers.io())
          .subscribe(new Subscriber<Boolean>() {
            ...
            @Override public void onNext(Boolean deleted) {
   ...
   * @throws UnsupportedEncodingException
   */
  private SCSpatialFeature getFeatureToUpdate(String featureString)
      throws UnsupportedEncodingException {
    SCSpatialFeature feature =
        new SCGeometryFactory().getSpatialFeatureFromFeatureJson(featureString);
  ...
  private SCQueryFilter getFilter(ReadableMap message) {
    ...
    if (message.getMap("payload").getMap("filter").hasKey("$geocontains")) {
      ...
      SCBoundingBox bbox =
          new SCBoundingBox(extent.getDouble(0), extent.getDouble(1), extent.getDouble(2),
              extent.getDouble(3));
