...
public class SCJavascriptBridgeHandler implements WebViewJavascriptBridge.WVJBHandler {
  ...
  public void handle(String data, final WebViewJavascriptBridge.WVJBResponseCallback jsCallback) {
    ...
    if (data == null && data.equals("undefined")) {
      ...
      if (command.equals(SCCommand.DATASERVICE_ACTIVESTOREBYID)) {
        ...
        try {
          dataStoreString =
              MAPPER.writeValueAsString(manager.getDataService().getStoreById(storeId));
      ...
      if (command.equals(SCCommand.DATASERVICE_UPDATEFEATURE)) {
        try {
          SCSpatialFeature featureToUpdate =
              getFeatureToUpdate(bridgeMessage.get("payload").get("feature").asText());
          manager.getDataService()
              ...
              .subscribe(new Subscriber<Boolean>() {
                ...
                @Override public void onNext(Boolean updated) {
  ...
  private SCQueryFilter getFilter(JsonNode payload) {
    ...
    try {
      List<Integer> points =
          MAPPER.readValue(bboxNode.traverse(), new TypeReference<List<Integer>>() {
          });
      SCBoundingBox bbox =
          new SCBoundingBox(points.get(0), points.get(1), points.get(2), points.get(3));
      SCQueryFilter filter = new SCQueryFilter(
          new SCPredicate(bbox, SCGeometryPredicateComparison.SCPREDICATE_OPERATOR_WITHIN));
      return filter;
    } catch (IOException e) {
      Log.e(LOG_TAG,
          "couldn't build filter...check the syntax of your bbox: " + bboxNode.textValue());
      e.printStackTrace();
    }
    return null;
  }
