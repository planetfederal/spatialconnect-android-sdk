...
public final class SQLiteSession {
  ...
  public boolean yieldTransaction(long sleepAfterYieldDelayMillis, boolean throwIfUnsafe,
    ...
    if (throwIfUnsafe) {
      ...
      if (mTransactionStack == null
          || mTransactionStack.mMarkedSuccessful
  ...
  public int executeForCursorWindow(String sql, Object[] bindArgs, CursorWindow window,
      int startPos, int requiredPos, boolean countAllRows, int connectionFlags,
      CancellationSignal cancellationSignal) {
    if (sql == null) {
      throw new IllegalArgumentException("sql must not be null.");
    }
    ...
    acquireConnection(sql, connectionFlags, cancellationSignal); // might throw
    try {
      return mConnection.executeForCursorWindow(sql, bindArgs, window, startPos, requiredPos,
