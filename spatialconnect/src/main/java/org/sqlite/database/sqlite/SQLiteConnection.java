...
public final class SQLiteConnection implements CancellationSignal.OnCancelListener {
  ...
  private static native void nativeBindLong(long connectionPtr, long statementPtr, int index,
      long value);

  private static native void nativeBindDouble(long connectionPtr, long statementPtr, int index,
      double value);

  private static native void nativeBindString(long connectionPtr, long statementPtr, int index,
      String value);

  private static native void nativeBindBlob(long connectionPtr, long statementPtr, int index,
      byte[] value);

  private static native void nativeResetStatementAndClearBindings(long connectionPtr,
      long statementPtr);
  ...
  private static native long nativeExecuteForCursorWindow(long connectionPtr, long statementPtr,
      CursorWindow win, int startPos, int requiredPos, boolean countAllRows);
  ...
  private void setLocaleFromConfiguration() {
    ...
    try {
      ...
      try {
        ...
        execute("INSERT INTO android_metadata (locale) VALUES(?)", new Object[] { newLocale },
            null);
      ...
      throw new SQLiteException(
          "Failed to change locale for db '" + mConfiguration.label + "' to '" + newLocale + "'.",
          ex);
    }
  }
          ...
          if (columnCount == 0) {
            ...
            for (int i = 0; i < columnCount; i++) {
              outStatementInfo.columnNames[i] =
                  nativeGetColumnName(mConnectionPtr, statement.mStatementPtr, i);
            }
          }
        }
      } finally {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          try {
            final long result =
                nativeExecuteForCursorWindow(mConnectionPtr, statement.mStatementPtr, window,
        ...
        if (mRecentOperations.endOperationDeferLog(cookie)) {
          mRecentOperations.logOperation(cookie, "window='"
              + window
              + "', startPos="
              + startPos
              + ", actualPos="
...
/*        }*/
  ...
  private DbStats getMainDbStatsUnsafe(int lookaside, long pageCount, long pageSize) {
    ...
    return new DbStats(label, pageCount, pageSize, lookaside, mPreparedStatementCache.hitCount(),
        mPreparedStatementCache.missCount(), mPreparedStatementCache.size());
  }
  ...
  private static final class Operation {
    ...
    public void describe(StringBuilder msg, boolean verbose) {
      ...
      if (mFinished) {
        ...
        msg.append(" started ").append(System.currentTimeMillis() - mStartTime).append("ms ago");
      }
  ...
  private final class PreparedStatementCache extends LruCache<String, PreparedStatement> {
    ...
    @Override protected void entryRemoved(boolean evicted, String key, PreparedStatement oldValue,
