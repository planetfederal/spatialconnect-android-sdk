...
public final class SQLiteConnectionPool implements Closeable {
  ...
  private void dispose(boolean finalized) {
    ...
    if (!finalized) {
      ...
      synchronized (mLock) {
        ...
        if (pendingCount != 0) {
          Log.i(TAG, "The connection pool for "
  ...
  private SQLiteConnection openConnectionLocked(SQLiteDatabaseConfiguration configuration,
    ...
    return SQLiteConnection.open(this, configuration, connectionId,
      ...
      if (!mIsOpen) {
        ...
        try {
          ...
          if (!waiter.mWantPrimaryConnection && !nonPrimaryConnectionNotAvailable) {
            connection = tryAcquireNonPrimaryConnectionLocked(waiter.mSql,
  ...
  private ConnectionWaiter obtainConnectionWaiterLocked(Thread thread, long startTime, int priority,
      boolean wantPrimaryConnection, String sql, int connectionFlags) {
