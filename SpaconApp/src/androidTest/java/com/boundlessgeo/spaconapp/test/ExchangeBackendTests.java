...
public class ExchangeBackendTests {
    ...
    public static void setUpBeforeClass() throws Exception {
        ...
        try {
            remoteConfigFile = File.createTempFile("exchange_remote.json", null, activity.getCacheDir());
    ...
    @Test
    public void testThatConfigIsFetchedAfterAuthenticating() {
        TestSubscriber authSubscriber = new TestSubscriber();
        sc.getAuthService().getLoginStatus().subscribe(authSubscriber);
        sc.getAuthService().authenticate("farpoint", "farpoint");
        Assert.assertEquals("The form store should have received form configs from the backend",
                1, sc.getDataService().getFormStore().getFormConfigs().size());
    }
}
